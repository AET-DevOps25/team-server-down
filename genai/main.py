...
@app.post("/completion", response_model=TextResponse)
async def complete_text(request: TextRequest):
    try:
        prompt = f"""Complete the following text naturally:
        {' '.join(request.user_text)}
        
        Provide a natural continuation that maintains the style and context.
        """
...
@app.post("/summarization", response_model=TextResponse)
...
@app.post("/rephrase", response_model=TextResponse)
